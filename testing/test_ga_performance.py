...
def test_ga_performance():
    ...
    try:
        ...
        if avg_time > 30:
            print("⚠️  WARNING: Very slow performance detected!")
            print("   Consider further reducing positions_count or population_size")
        elif avg_time > 10:
            print("⚠️  CAUTION: Slow performance detected")
...
def test_stockfish_performance():
    ...
    try:
        ...
        if avg_eval_time > 1.0:
            print("⚠️  WARNING: Stockfish evaluations are very slow!")
        elif avg_eval_time > 0.5:
            print("⚠️  CAUTION: Stockfish evaluations are slow")
        else:
            print("✅ Stockfish performance looks good")
        
        stockfish.close()
        print("✅ Stockfish test completed!")
        
    except Exception as e:
        print(f"❌ Error during Stockfish test: {e}")
        import traceback
        traceback.print_exc()
        return False
