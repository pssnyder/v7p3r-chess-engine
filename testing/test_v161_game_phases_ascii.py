...
def test_initialization():
    ...
    try:
        ...
        if book_move_uci:
            ...
            print(f"[OK] Opening book working - suggests: {board.san(move)} ({book_move_uci})")
        else:
            print("[WARN] No opening book move found")
        
        # Test position evaluation
        print("\n[1c] Testing evaluation function...")
        score = engine._evaluate_position(board)
        print(f"[OK] Starting position evaluation: {score}cp")
...
def test_opening_phase(engine):
    ...
    for name, fen in test_positions:
        ...
        # Show evaluation
        score = engine._evaluate_position(board)
        print(f"    [EVAL]  Evaluation: {score:+d}cp")
...
def test_middlegame_phase(engine):
    ...
    for name, fen, description in test_positions:
        ...
        try:
            ...
            print(f"    [WARN] Error calculating bonus: {e}")
        
        # Full evaluation
        score = engine._evaluate_position(board)
        print(f"    [EVAL]  Total evaluation: {score:+d}cp")
...
def test_endgame_phase(engine):
    ...
    for name, fen, description in test_positions:
        ...
        best_move = engine.get_best_move(board, time_limit=2.0)
        if best_move:
