    ...
    try:
        ...
        for i, position in enumerate(test_positions):
            ...
            if best_move == expected_move:
                print(f"✅ PASS: Engine found the expected checkmate move: {expected_move}")
                successful_tests += 1
            else:
                print(f"❌ FAIL: Engine did not find the expected move {expected_move}")
                # Make the move and see if it leads to checkmate anyway
                if best_move != chess.Move.null():
                    board_copy = board.copy()
                    board_copy.push(best_move)
                    if board_copy.is_checkmate():
                        print(f"✅ However, the move {best_move} also leads to checkmate!")
                        successful_tests += 1
                    else:
                        ...
                        if board_copy2.is_checkmate() and not board_copy.is_checkmate():
                            print(f"⚠️ The engine missed a direct checkmate!")
        ...
        if successful_tests == total_tests:
            print("🎉 All checkmate tests passed! The engine is detecting checkmates correctly.")
        else:
            print(f"⚠️ {total_tests - successful_tests} tests failed. The engine still needs improvement in checkmate detection.")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    if success:
        print("🎯 CHECKMATE DETECTION VALIDATED SUCCESSFULLY!")
    else:
