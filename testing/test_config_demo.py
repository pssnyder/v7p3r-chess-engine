...
def main():
    ...
    print(f"   ✓ Available config sections: {list(config_manager.get_config().keys())}")
    
    # 2. Module-Specific Configuration Access
    print("\n2. Module-Specific Configuration Access:")
    engine_config = config_manager.get_engine_config()
    print(f"   ✓ Engine depth: {engine_config.get('depth', 'Not set')}")
    print(f"   ✓ Engine search algorithm: {engine_config.get('search_algorithm', 'Not set')}")
    
    stockfish_config = config_manager.get_stockfish_config()
    print(f"   ✓ Stockfish ELO: {stockfish_config.get('elo_rating', 'Not set')}")
    print(f"   ✓ Stockfish skill level: {stockfish_config.get('skill_level', 'Not set')}")
    ...
    print("   ✓ Created NN-specific configuration")
    print(f"   ✓ NN batch size: {nn_config.get_v7p3r_nn_config().get('batch_size')}")
    print(f"   ✓ NN learning rate: {nn_config.get_v7p3r_nn_config().get('learning_rate')}")
    ...
    print("   ✓ Created GA-specific configuration")
    print(f"   ✓ GA population size: {ga_config.get_v7p3r_ga_config().get('population_size')}")
    print(f"   ✓ GA generations: {ga_config.get_v7p3r_ga_config().get('generations')}")
    ...
    print("   ✓ Created runtime configuration with overrides")
    print(f"   ✓ Runtime engine depth: {runtime_config.get_engine_config().get('depth')}")
    print(f"   ✓ Runtime max depth: {runtime_config.get_engine_config().get('max_depth')}")
    print(f"   ✓ Runtime Stockfish ELO: {runtime_config.get_stockfish_config().get('elo_rating')}")
    print(f"   ✓ Runtime game count: {runtime_config.get_game_config().get('game_count')}")
    ...
    for module_name in ['engine', 'stockfish', 'game', 'nn', 'ga', 'rl']:
        try:
            ...
            print(f"   ✓ {module_name.upper()} config loaded successfully")
        except ValueError as e:
            print(f"   ✗ {module_name.upper()} config failed: {e}")
