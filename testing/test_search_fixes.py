...
def test_search_fixes():
    ...
    try:
        ...
        if abs(white_eval + black_eval) > 0.1:
            print("⚠️  WARNING: Evaluations are not properly negated")
        else:
            print("✓ Evaluation perspective consistency verified")
        ...
        if start_position.is_legal(move_white):
            print("✓ Simple search returned legal move for White")
        else:
            print("❌ Simple search returned illegal move for White")
            return False
        ...
        if black_board.is_legal(move_black):
            print("✓ Simple search returned legal move for Black")
        else:
            print("❌ Simple search returned illegal move for Black")
            return False
        ...
        try:
            ...
            if start_position.is_legal(move_minimax):
                print("✓ Minimax search returned legal move")
            else:
                print("❌ Minimax search returned illegal move")
                return False
                
        except Exception as e:
            print(f"❌ Minimax search failed: {e}")
            return False
        
        print("\n" + "=" * 60)
        print("✅ ALL SEARCH LOGIC FIXES VERIFIED!")
