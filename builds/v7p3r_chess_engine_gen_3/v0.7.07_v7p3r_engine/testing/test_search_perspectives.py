    ...
    for desc, fen in test_positions:
        ...
        if "advantage" in desc:
            ...
            if (white_eval > 0) != expect_white_positive:
                print(f"  Γ¥î White direct evaluation inconsistent: {white_eval:+.2f}")
            else:
                print(f"  Γ£ô White direct evaluation consistent: {white_eval:+.2f}")
                
            if (black_eval > 0) != (not expect_black_negative):
                print(f"  Γ¥î Black direct evaluation inconsistent: {black_eval:+.2f}")
            else:
                print(f"  Γ£ô Black direct evaluation consistent: {black_eval:+.2f}")
...
if __name__ == "__main__":
    ...
    if test_search_perspectives():
        print("\nΓ£à Search perspective test passed!")
    else:
        print("\nΓ¥î Search perspective test failed!")
