...
class ChessGame:
    def __init__(self):
        ...
        # Set up evaluation
        self.evaluator = EvaluationEngine(self.board, depth=self.depth)
    ...
    def ai_move(self):
        ...
        legal_indices = [self.move_encoder.encode_move(m) for m in legal_moves]
        legal_mask = torch.zeros_like(logits)
        legal_mask[0, legal_indices] = 1.0
        
        # Apply mask and select best move
        masked_logits = logits * legal_mask
        best_idx = torch.argmax(masked_logits).item()
        
        # Verify move is legal
        best_move_uci = self.move_encoder.decode_move(best_idx)
        if best_move_uci in legal_moves:
            return best_move_uci
        
        # Fallback to highest probability legal move
        legal_logits = logits[0, legal_indices]
        best_legal_idx = torch.argmax(legal_logits).item()
        return legal_moves[best_legal_idx]
