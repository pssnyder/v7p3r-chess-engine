    ...
    if board.is_check():
        ...
        if capture_moves and king_moves:
            ...
            if capture_index < king_index:
                print("Γ£ô SUCCESS: Capture of checking piece prioritized over king moves!")
                return True
            else:
                print("Γ£ù FAIL: King moves still prioritized over capture")
                return False
        else:
    ...
    if board.is_check():
        ...
        if capture_move in board.legal_moves and king_move in board.legal_moves:
            ...
            if capture_tempo > king_tempo:
                print("Γ£ô SUCCESS: Capture gets higher tempo score!")
                return True
            else:
                print("Γ£ù FAIL: King move still gets higher tempo")
                return False
        else:
...
if __name__ == "__main__":
    ...
    if test1_pass and test2_pass:
        print("Γ£ô ALL TESTS PASSED - Enhancement working correctly!")
    else:
        print("Γ£ù SOME TESTS FAILED - Enhancement needs adjustment")
