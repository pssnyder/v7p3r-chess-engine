...
try:
    with open("config/chess_game_config.yaml", "r") as config_file:
        ...
        # unique_search_algorithms = sorted(list(set(search_algorithms + ['V7P3R', 'Stockfish']))) # No longer needed for UI
...
def update_ab_testing_section(_, selected_metric): # Removed white_search_algorithms, black_search_algorithms
    ...
    for _, game_row in df_games_raw.iterrows():
        ...
        white_is_v7p3r = game_row.get('white_engine_type') == 'V7P3R'
        black_is_v7p3r = game_row.get('black_engine_type') == 'V7P3R'
...
def update_static_metrics(_):
    ...
    for _, row in df_games_raw.iterrows():
        white_is_v7p3r = row.get('white_engine_type') == 'V7P3R'
        black_is_v7p3r = row.get('black_engine_type') == 'V7P3R'
    ...
    for _, row in df_v7p3r_games.iterrows():
        ...
        white_is_v7p3r_and_included = row.get('white_engine_type') == 'V7P3R' and not row.get('exclude_white_from_metrics', False)
        black_is_v7p3r_and_included = row.get('black_engine_type') == 'V7P3R' and not row.get('exclude_black_from_metrics', False)
