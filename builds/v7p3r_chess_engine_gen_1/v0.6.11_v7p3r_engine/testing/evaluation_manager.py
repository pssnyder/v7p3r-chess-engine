...
# Injests and organizes chess engine data for analysis and visualization.
...
class EvaluationManager:
    """Functions to organize and evaluate engine feedback and game results.
    Raw data formats: 
     - Log files from engine evaluations
     - CSV files for puzzle solutions
     - PGN files for game results
     - JSON files for evaluation records
    """
    def __init__(self, logging_dir="logging", games_dir="games", metrics_dir="metrics"):
        self.logging_dir = logging_dir
        self.games_dir = games_dir
        self.metrics_dir = metrics_dir

    def collect_files(self):
        """Recursively collect all files in logging, games, metrics directories."""
        files = []
        for folder in [self.logging_dir, self.games_dir, self.metrics_dir]:
    ...
    def parse_pgn(self, filepath):
        """Extract result and moves from a PGN file."""
        result, moves = None, []
    ...
    def match_game_configs(self):
        """Match PGN games with their corresponding config YAMLs by timestamp."""
        files = self.collect_files()
        config_map = {}
        game_map = {}
        for f in files:
            if re.match(r".*eval_config_\d{8}_\d{6}\.ya?ml$", f):
                m = re.search(r"eval_config_(\d{8}_\d{6})\.ya?ml$", f)
                if m:
        ...
        return game_map, config_map

    def build_consolidated_dataset(self):
        """Build a unified DataFrame from all sources for analytics."""
        game_map, config_map = self.match_game_configs()
        records = []
        for ts, game_file in game_map.items():
            config_file = config_map.get(ts)
            game_info = self.parse_pgn(game_file)
            config_info = self.load_yaml(config_file) if config_file else {}
            record = {
                "timestamp": ts,
                "game_file": game_file,
        ...
        # Optionally, merge with metrics/logs/JSON as needed
        return df

    def correlate_config_with_results(self, df, config_param):
        """Return a DataFrame showing correlation between a config param and game results."""
        if df[config_param].apply(lambda x: isinstance(x, dict)).any():
            df[config_param] = df[config_param].apply(lambda x: str(x) if isinstance(x, dict) else x)
        return df.groupby([config_param, "result"]).size().unstack(fill_value=0)

# Example usage:
if __name__ == "__main__":
    manager = EvaluationManager()
    df = manager.build_consolidated_dataset()
    print(df)

# Evaluation Manager Configuration and Example Data
...
# JSON files contain verified evaluation records for validating engine calculations and accuracy
# These files are not produced by the V7P3R engine, but are an output of Lichess.org's analysis of positions and use for verification cross reference only.
