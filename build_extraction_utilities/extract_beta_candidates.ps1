# Beta Candidate Extraction Script
# This script extracts core engine files from each beta-candidate tag into organized build directories
...
Write-Host "Starting extraction of $($betaCandidates.Count) beta candidates..." -ForegroundColor Green
...
foreach ($candidate in $betaCandidates) {
    ...
    Write-Host "Processing $tag -> $buildDirName" -ForegroundColor Yellow
            ...
            if (!$shouldExclude) {
                ...
                try {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        ...
                        Write-Host "  ✗ Git error for: $file ($gitOutput)" -ForegroundColor Red
                    }
                } catch {
    ...
    $buildInfo = @{
        ...
        coreFiles = (Get-ChildItem -Path $buildPath -Recurse -File).Name
    } | ConvertTo-Json -Depth 3
    
    $buildInfo | Out-File -FilePath (Join-Path $buildPath "BUILD_INFO.json") -Encoding UTF8
    
    Write-Host "  Created build: $buildDirName" -ForegroundColor Green
}

Write-Host "`nExtraction complete! Created builds for $($betaCandidates.Count) beta candidates." -ForegroundColor Green
