    ...
    for test in test_positions:
        ...
        if 'best_move' in test:
            ...
            if best_move == expected_move:
                logger.info(f"Γ£ô Engine correctly found the expected move: {expected_move}")
            else:
                logger.info(f"Γ£ù Engine did not find the expected move. Found {best_move} instead of {expected_move}")
        ...
        if best_move != chess.Move.null():
            ...
            if board_after_move.is_checkmate():
                logger.info("Γ£ô The move leads to immediate checkmate!")
            
            # Check if it avoided checkmate
            elif board.is_check() and not board_after_move.is_check():
                logger.info("Γ£ô The move successfully escapes check")
            
            # Check if it captures a piece
            elif board.is_capture(best_move):
                logger.info("Γ£ô The move captures a piece")
            
            # Check if it gives check
            elif board_after_move.is_check():
                logger.info("Γ£ô The move gives check")
            
            logger.info(f"Board after move: {board_after_move.fen()}")
        else:
            logger.info("Γ£ù Engine couldn't find a valid move")
