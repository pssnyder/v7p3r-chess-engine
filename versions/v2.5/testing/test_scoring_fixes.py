    ...
    try:
        ...
        for desc, board in test_positions:
            ...
            for func_name in functions_to_test:
                try:
                    ...
                    if board.fen() != original_fen:
                        print(f"  Γ¥î {func_name}: BOARD MUTATED!")
                        return False
                    else:
                        print(f"  Γ£à {func_name}: White={white_score:.2f}, Black={black_score:.2f}")
                        
                except Exception as e:
                    print(f"  Γ¥î {func_name}: ERROR - {e}")
                    return False
            ...
            try:
                ...
                print(f"  ≡ƒôè Total scores: White={total_white:.2f}, Black={total_black:.2f}")
                
                # Verify board wasn't mutated by full calculation
                if board.fen() != original_fen:
                    print(f"  Γ¥î calculate_score: BOARD MUTATED!")
                    return False
                    
            except Exception as e:
                print(f"  Γ¥î calculate_score: ERROR - {e}")
                return False
        
        print(f"\n≡ƒÄë ALL TESTS PASSED!")
        print("Γ£à No board mutations detected")
        print("Γ£à All functions work with static evaluation")
        print("Γ£à Performance should be significantly improved")
        
        return True
        
    except Exception as e:
        print(f"Γ¥î Test setup failed: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if success:
        print("\n" + "=" * 60)
        print("≡ƒÄ» SCORING FIXES VALIDATED SUCCESSFULLY!")
        print("Your engine should now:")
        print("  ΓÇó Make faster evaluations (no legal move iteration)")
        print("  ΓÇó Find checkmates more reliably")
        print("  ΓÇó Make more consistent positional decisions") 
        print("  ΓÇó Avoid erratic play caused by board mutations")
    else:
        print("\n" + "=" * 60)
