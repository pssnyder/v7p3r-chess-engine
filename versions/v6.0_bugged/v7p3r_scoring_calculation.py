...
class V7P3RScoringCalculation:
    ...
    def _has_pawn_support_on_file(self, board: chess.Board, file: int, color: chess.Color) -> bool:
        """Check if there's a pawn of the same color on the specified file."""
        if file < 0 or file > 7:
            return False
    ...
    def _analyze_doubled_pawns(self, board: chess.Board, color: chess.Color) -> float:
        """Analyze doubled and tripled pawns with context."""
        score = 0.0
        ...
        for file in range(8):
            ...
            for rank in range(8):
                ...
                piece = board.piece_at(square)
                if piece and piece.piece_type == chess.PAWN and piece.color == color:
                    pawns_on_file.append((square, rank))
    ...
    def _has_enemy_pawns_on_file(self, board: chess.Board, file: int, our_color: chess.Color) -> bool:
        """Check if enemy has pawns on specified file."""
    ...
    def _detect_weak_pawn_squares(self, board: chess.Board, color: chess.Color) -> float:
        """Detect weak squares in pawn structure that enemy can exploit."""
        score = 0.0
    ...
    def _can_control_with_pawns(self, board: chess.Board, square: chess.Square, color: chess.Color) -> bool:
        """Check if we can control a square with our pawns."""
        file = chess.square_file(square)
        rank = chess.square_rank(square)
        
        # Check if pawns on adjacent files can advance to control this square
        for file_offset in [-1, 1]:
            pawn_file = file + file_offset
            if 0 <= pawn_file <= 7:
                # Look for pawns that could advance to control the square
                for pawn_rank in range(8):
                    pawn_square = chess.square(pawn_file, pawn_rank)
                    piece = board.piece_at(pawn_square)
                    if piece and piece.piece_type == chess.PAWN and piece.color == color:
                        # Check if this pawn could potentially control the target square
                        if color == chess.WHITE:
                            can_control = pawn_rank < rank and abs(pawn_rank + 1 - rank) <= 1
                        else:
                            can_control = pawn_rank > rank and abs(pawn_rank - 1 - rank) <= 1
