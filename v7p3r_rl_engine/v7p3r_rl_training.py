...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logging/v7p3r_rl_training.log'),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    return logging.getLogger("v7p3r_rl_training")
...
def run_training_session(engine: v7p3rRLEngine, config: Dict[str, Any], logger: logging.Logger):
    ...
    logger.info(f"Starting training session: {total_episodes} episodes, batch size {batch_size}")
    ...
    try:
        for episode_batch in range(0, total_episodes, batch_size):
            ...
            # Train batch
            logger.info(f"Training episodes {episode_batch + 1}-{episode_batch + current_batch_size}")
            engine.train_self_play(current_batch_size, current_batch_size)
            
            # Periodic validation
            if (episode_batch + current_batch_size) % validation_frequency == 0:
                logger.info("Running Stockfish validation...")
                validation_stats = engine.validate_performance(num_games=10)
                
                if validation_stats:
                    logger.info(f"Validation results: {validation_stats['wins']}W-{validation_stats['losses']}L-{validation_stats['draws']}D "
            ...
            logger.info(f"Progress: {episode_batch + current_batch_size}/{total_episodes} episodes "
        ...
        # Final validation
        logger.info("Running final validation against Stockfish...")
        final_validation = engine.validate_performance(num_games=20)
        if final_validation:
            logger.info(f"Final validation: {final_validation['wins']}W-{final_validation['losses']}L-{final_validation['draws']}D "
        ...
        logger.info(f"  Game results: {stats['wins']}W-{stats['losses']}L-{stats['draws']}D")
        logger.info(f"  Average reward: {stats['total_reward']/max(stats['episodes'], 1):.3f}")
        if stats['stockfish_games'] > 0:
            sf_wr = stats['stockfish_wins'] / stats['stockfish_games']
            logger.info(f"  Stockfish performance: {stats['stockfish_wins']}/{stats['stockfish_games']} ({sf_wr:.2%})")
...
def main():
    ...
    logger.info("Starting v7p3r RL training script")
    ...
    # Initialize engine
    logger.info("Initializing v7p3r RL engine...")
    ...
    if args.load_model:
        if os.path.exists(args.load_model):
            ...
            logger.warning(f"Pre-trained model not found: {args.load_model}")
