...
class v7p3rScore:
    ...
    def calculate_score(self, board: chess.Board, color: chess.Color, endgame_factor: float = 0.0) -> float:
        ...
        if v7p3r_thinking:
            if self.logger and self.monitoring_enabled and self.verbose_output_enabled:
                self.logger.info(f"[Scoring Calc] Piece attacks score for {color_name}: {piece_attacks_score:.3f} (Ruleset: {self.ruleset_name})")
            if self.verbose_output_enabled:
        ...
        if v7p3r_thinking:
            if self.logger and self.monitoring_enabled and self.verbose_output_enabled:
                self.logger.info(f"[Scoring Calc] Tempo score for {color_name}: {tempo_score:.3f} (Ruleset: {self.ruleset_name})")
            if self.verbose_output_enabled:
