...
class V7P3REngine:
    def __init__(self, config_file="config.json"):
        ...
        # Initialize opening book only if enabled and the opening data directory exists
        use_book = engine_config.get('use_opening_book', False)
        book_dir = os.path.join(os.path.dirname(__file__), 'pgn_opening_data')
        if use_book and os.path.exists(book_dir):
            try:
                self.book = OpeningBook(opening_dir=book_dir)
            except Exception:
                # If book fails to load, proceed without it
                self.book = None
        else:
            self.book = None
