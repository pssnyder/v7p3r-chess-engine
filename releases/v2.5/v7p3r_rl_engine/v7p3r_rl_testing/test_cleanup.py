...
def test_stockfish_cleanup():
    ...
    try:
        ...
        if final_count <= initial_count:
            print("Γ£à Stockfish cleanup test PASSED!")
            return True
        else:
            print("Γ¥î Stockfish cleanup test FAILED - processes not cleaned up!")
            return False
            
    except Exception as e:
        print(f"Γ¥î Error in cleanup test: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_ga_cleanup():
    ...
    try:
        ...
        if final_count <= initial_count:
            print("Γ£à GA cleanup test PASSED!")
            return True
        else:
            print("Γ¥î GA cleanup test FAILED!")
            return False
            
    except Exception as e:
        print(f"Γ¥î Error in GA cleanup test: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    ...
    print(f"\n=== Test Summary ===")
    print(f"Stockfish cleanup: {'Γ£à' if stockfish_ok else 'Γ¥î'}")
    print(f"GA cleanup: {'Γ£à' if ga_ok else 'Γ¥î'}")
    
    if stockfish_ok and ga_ok:
        print("\n≡ƒÄë All cleanup tests passed! Stockfish processes should now terminate properly.")
    else:
        print("\nΓÜá∩╕Å  Some cleanup tests failed. Check the issues above.")
