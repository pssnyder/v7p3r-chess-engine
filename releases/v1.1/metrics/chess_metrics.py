...
try:
    with open("chess_game.yaml", "r") as config_file:
        ...
        # unique_ai_types = sorted(list(set(AI_TYPES + ['V7P3R', 'Stockfish']))) # No longer needed for UI
...
def update_ab_testing_section(_, selected_metric): # Removed white_ai_types, black_ai_types
    ...
    # Get all moves made by V7P3R
    all_v7p3r_moves_raw = metrics_store.get_filtered_move_metrics(
        white_ai_types=['V7P3R'], # Only V7P3R as white
        black_ai_types=['V7P3R'], # Only V7P3R as black
    ...
    for _, game_row in df_games_raw.iterrows():
        ...
        white_is_v7p3r = game_row.get('white_ai_type') == 'V7P3R'
        black_is_v7p3r = game_row.get('black_ai_type') == 'V7P3R'
...
def update_static_metrics(_):
    ...
    for _, row in df_games_raw.iterrows():
        white_is_v7p3r = row.get('white_ai_type') == 'V7P3R'
        black_is_v7p3r = row.get('black_ai_type') == 'V7P3R'
    ...
    for _, row in df_v7p3r_games.iterrows():
        ...
        white_is_v7p3r_and_included = row.get('white_ai_type') == 'V7P3R' and not row.get('exclude_white_from_metrics', False)
        black_is_v7p3r_and_included = row.get('black_ai_type') == 'V7P3R' and not row.get('exclude_black_from_metrics', False)
