...
class V7P3RCleanEngine:
    ...
    def _score_move_enhanced(self, board: chess.Board, move: chess.Move, 
        ...
        """V8.2: Enhanced move scoring aligned with evaluation priorities"""
        score = 0.0
        
        # 1. Mate detection (highest priority)
        board.push(move)
        if board.is_checkmate():
            board.pop()
            return 200000.0  # Mate found
        board.pop()
        ...
        # 6. History heuristic (if enabled)
        if options.use_history_heuristic:
