...
class EnhancedScoringCollector:
    ...
    def collect_detailed_scoring(self, scoring_calculator, board: chess.Board, color: chess.Color) -> Dict[str, float]:
        ...
        try:
            ...
            if self.logger:
                self.logger.info(f"Enhanced scoring collection complete. Total components: {len(detailed_scores)}")
    ...
    def _safe_call(self, method, *args) -> float:
        ...
        try:
            ...
            if self.logger:
                self.logger.info(f"Error calling {method.__name__}: {e}")
            return 0.0
    ...
    def extract_scoring_dictionary(self, scoring_calculator) -> Dict[str, Any]:
        ...
        try:
            if hasattr(scoring_calculator, 'scoring') and isinstance(scoring_calculator.scoring, dict):
                ...
                if self.logger:
                    self.logger.info(f"Extracted scoring dictionary with {len(scoring_dict)} components")
            else:
                if self.logger:
