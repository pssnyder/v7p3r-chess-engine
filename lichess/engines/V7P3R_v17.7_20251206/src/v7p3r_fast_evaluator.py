#!/usr/bin/env python3
"""
V7P3R Fast Evaluator - v17.7 Anti-Draw Endgame Technique
...
v17.7 ENHANCEMENTS (Anti-Draw Measures):
- King-Edge Driving: +10cp × distance bonus when material advantage >600cp
  * Pushes opponent king toward edge in winning endgames
  * Critical for K+R vs K, K+Q vs K, K+R+B vs K mates
  * Fixes game e68K458O issue (drew R+B vs K instead of mating)
- Complements search-level threefold avoidance and mate verification
- Expected cost: <5μs per endgame eval (negligible)
...
class V7P3RFastEvaluator:
    ...
    def evaluate(self, board: chess.Board) -> int:
        ...
        # V17.7 bonuses (anti-draw endgame technique)
        v17_7_bonus = 0
        if is_endgame:
            v17_7_bonus = self._calculate_v17_7_bonuses(board, material_score)
        
        # Combine scores: 60% PST + 40% Material + Bonuses
        combined_score = int(pst_score * 0.6 + material_score * 0.4 + middlegame_bonus + endgame_bonus + v17_6_bonus + v17_7_bonus)
    ...
    def _calculate_v17_7_bonuses(self, board: chess.Board, material_score: int) -> int:
        ...
        # Only apply when significantly ahead in material (>400cp = roughly up a minor piece or more)
