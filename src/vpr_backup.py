...
class VPREngine:
    ...
    def search(self, board: chess.Board, time_limit: float = 3.0, 
               depth: Optional[int] = None) -> chess.Move:
        """
        TAL-BOT SPEED-OPTIMIZED search for 6-10 ply forest vision
        ...
        # Calculate time allocation (use 85% for deeper search)
        target_time = time_limit * 0.85
        
        # SPEED-OPTIMIZED iterative deepening for forest vision
        max_depth = depth if depth else 12  # Aim higher!
        best_move = legal_moves[0]
        best_score = -999999
        ...
        for current_depth in range(1, max_depth + 1):
            ...
            # Fast search at current depth
            score = -999999
            current_best = best_move
            current_pv = []
            time_exceeded = False
            
            # Use simplified move ordering for speed
            ordered_moves = self._order_moves_fast(board, legal_moves)
            ...
            for move in ordered_moves:
                ...
                # SPEED-OPTIMIZED negamax
                move_score = -self._negamax_speed(board, current_depth - 1, -999999, 999999, target_time)
                ...
                if move_score > score:
                    score = move_score
                    current_best = move
                    current_pv = [move]  # Simple PV
            
            # Update best move only if we completed this depth
            if not time_exceeded:
                best_move = current_best
                best_score = score
                self.principal_variation = current_pv[:]
                
                # UCI info output with PV
                elapsed = time.time() - self.search_start_time
                nps = int(self.nodes_searched / elapsed) if elapsed > 0 else 0
                
                pv_string = str(current_pv[0]) if current_pv else str(best_move)
    ...
    def _order_moves_fast(self, board: chess.Board, moves: List[chess.Move]) -> List[chess.Move]:
        """Fast move ordering for speed optimization"""
        move_scores = []
        
        for move in moves:
            score = 0
            
            # Captures (simple MVV-LVA)
            ...
            # Basic piece value consideration
            piece = board.piece_at(move.from_square)
            if piece:
                score += [0, 10, 30, 30, 50, 90, 900][piece.piece_type]
    ...
    def _negamax_speed(self, board: chess.Board, depth: int, alpha: float, beta: float, time_limit: float) -> float:
        """Speed-optimized negamax search"""
        self.nodes_searched += 1
        ...
        for move in ordered_moves:
            board.push(move)
            eval_score = -self._negamax_speed(board, depth - 1, -beta, -alpha, time_limit)
            board.pop()
    ...
    def _quick_evaluate(self, board: chess.Board) -> float:
        """Ultra-fast evaluation for speed optimization"""
        score = 0
    ...
    def _order_moves_simple(self, board: chess.Board, moves: List[chess.Move]) -> List[chess.Move]:
        ...
        high_priority, low_priority = self._get_priority_pieces_fast(board, board.turn)
        ...
        for move in moves:
            ...
            if from_square in high_priority:
                ...
                true_value = self._calculate_piece_true_value_fast(board, from_square, board.turn)
    ...
    def _calculate_piece_true_value_fast(self, board: chess.Board, square: chess.Square, color: chess.Color) -> int:
        """
        SPEED-OPTIMIZED: Calculate TRUE piece value with minimal overhead
        
        This is the core of TAL-BOT but optimized for 6-10 ply depth performance.
        Cache-friendly and minimal python-chess calls.
        ...
        Returns:
            True value = attacks_count + legal_moves_count (optimized)
        """
        piece = board.piece_at(square)
        if not piece or piece.color != color:
            return 0
        
        # Fast attack count using cached attacks
        attacks = board.attacks(square)
        attack_count = len(attacks)
        
        # Simplified legal move count - avoid generating all moves
        # Use piece type and position to estimate mobility
        legal_moves_estimate = 0
        
        if piece.piece_type == chess.PAWN:
            legal_moves_estimate = 1 + (1 if attack_count > 0 else 0)  # Forward + captures
        elif piece.piece_type == chess.KNIGHT:
            legal_moves_estimate = min(attack_count, 8)  # Max 8 knight moves
        elif piece.piece_type == chess.BISHOP:
            legal_moves_estimate = min(attack_count, 14)  # Diagonal mobility
        elif piece.piece_type == chess.ROOK:
            legal_moves_estimate = min(attack_count, 14)  # Rank/file mobility
        elif piece.piece_type == chess.QUEEN:
            legal_moves_estimate = min(attack_count, 27)  # Queen mobility
        elif piece.piece_type == chess.KING:
            legal_moves_estimate = min(attack_count, 8)  # King moves
        
        true_value = attack_count + legal_moves_estimate
        
        # Quick tactical bonus
        if board.is_attacked_by(not color, square):
            true_value += 3  # Under attack - needs attention
        
        return true_value
    
    def _get_priority_pieces_fast(self, board: chess.Board, color: chess.Color) -> tuple:
        """
        SPEED-OPTIMIZED: Get priority pieces for tactical focus
        ...
        # Only scan pieces of our color (faster iteration)
        for square in board.pieces(chess.PAWN, color) | board.pieces(chess.KNIGHT, color) | \
            ...
            true_value = self._calculate_piece_true_value_fast(board, square, color)
            piece_values.append((true_value, square))
        
        if len(piece_values) < 3:
            return [sq for _, sq in piece_values], []
        
        # Sort by true value (highest first)
        piece_values.sort(key=lambda x: x[0], reverse=True)
        
        # Take top 2 and bottom 2 for focus
        high_priority = [sq for _, sq in piece_values[:2]]
        low_priority = [sq for val, sq in piece_values if val <= 2][-2:]
    ...
    def _evaluate_side_dynamic(self, board: chess.Board, color: chess.Color) -> float:
        ...
        high_priority, low_priority = self._get_priority_pieces_fast(board, color)
